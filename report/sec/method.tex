...
Not all defined parameters in our model had experimentally measured values.  Not a single species with published measurements on every biological parameter we defined.  Luckily there was at least one available value for each biological parameter in our model.  Hence we used a percentage range of known parameters as guide.  These guides were to narrow down the unknown ranges of biological parameters with limited data.  An uniform prior was to set sampling points within the parameter ranges of each biological parameters.  Ranges of each biological parameter was listed below and details of how we determined each biological parameter range was in the appendix.
...
Within the parameter ranges using the uniform prior, we selected 11 values in the regular interval for each parameter using R (v4.0.2).  Next we randomly arranged the values for each parameter so that each parameter combination was unique.  This step was called a Latin Hypercube Sampling (LHS).  To ensure the study’s reproducibility, we had set the seed for random numbers to ``20192020”.  500 unique LHS sets of parameter combinations and 200 harvest rates (for continuous harvest systems, $x$) of regular interval was applied.  Harvest values ranged from 1 to 19901 \dayU.  Harvest intervals (for destructive harvest systems, $T$, unit: day) was defined as
\begin{equation}
    T = x-1
    \label{eq:TvsX}
\end{equation}
Each system contained 1.1 million ($11\times500\times200$) scenarios for simulation.  An extra log-spaced time ($T$ = 0-100) for destructive systems was for investigating system behaviour before stability.

\subsection{Model experiment}
C (compiler gcc v7.5.0) was for analytical calculations on continuous harvest systems (equilibrium 3\&4 in Table \ref{t:eqm}).  Yield flux was defined by $x\cdot C$ in Eq.\ref{eq:PBH}, and hence the calculation showed \PoH\ systems were independent of harvest rate (Table \ref{t:eqm}).

Python3 SciPy (v1.2.3) package ``odeint" function was for numerical integration on destructive harvest systems (Eq.\ref{eq:PBN} \& \ref{eq:PoN}).  We used Eq.\ref{eq:PBH} by tweaking harvest rate parameter and initial densities because destructive harvest systems were variations from Eq.\ref{eq:PBH}.  Setting $x=0$, we ran \PBN\ systems by initial densities of $C=P=B=1$ \den\ and \PoN\ systems by $C=P=1$ and $B=0$ \den.  We defined the daily yield flux as the average yield difference in each ``wait and replace" cycle shown below:
\begin{equation}
...
We performed pairwise Wilcox signed rank tests (R v4.0.2) with bonferroni p-value adjustment and no paired situations on the four systems.  Effect of \bac\ during early system development was observed through plotting each carbon pool against the log-spaced time for destructive harvest systems.  Effect of \bac\ on median yield on each harvest mode were graphs of median yield against log harvest interval/rate.  Effect of \bac\ on yield distributions were boxplots of selected harvest interval/rate grouped by harvest mode.  Influence of biological features were a series of two boxplots on the group of harvest interval/rate containing maximum yield in log-space, comparing the effect of \bac\ and harvest mode respectively.
