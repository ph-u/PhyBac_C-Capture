...
Solving the ODE system analytically (i.e. integration by SciPy (v.1.4.1) ``odeint" function) showed a stable equilibrium mostly regardless to the initial value.  Solving the ODE system numerically (setting $\dC=\dP=\dB=0$ and solve the system of unknowns by SymPy (v.1.0.7) in Julia-lang (v.1.3.1)) revealed three alternative equlibrium positions but only one of them contained all three carbon pools with non-zero densities.  Both methods gave similar (values converged to 2 d.p.) equilibrium positions when using the same set of parameter values selected at random within the ranges in Table\ref{varInTab}.
