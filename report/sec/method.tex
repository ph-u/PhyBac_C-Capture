...
We performed pairwise Wilcox signed rank tests on all necessary occasions (in R) with bonferroni p-value adjustment and no paired situations to check on their difference in medians.  Since harvest rates and harvest intervals were similar in logic but different in units, we separately selected three harvest values differed in magnitude to compare the destructive/continuous harvest systems.  We also observed influence of each biological parameter by plotting log yield fluxes against each of them.
