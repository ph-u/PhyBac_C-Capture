...
numScenario=`wc -l ../data/scenario.csv|cut -f 5 -d " "` ## get number of sampled scenarios

##### looping calculation #####
counterX=1
for i0 in `seq ${minX} ${intX} ${maxX}`;do ## loop over sequence of harvest rate
    if [ $((${counterX}%10)) -eq 1 ];then
        curX=`echo "${counterX}/(${samX}+1)*100"|bc -l|cut -f 1 -d "."`
        echo -e "continuous: harvest rate = ${i0}, ${curX}% done; `date`"
    fi
    for i1 in `seq 2 ${numScenario}`;do ## loop over sampled scenarios
